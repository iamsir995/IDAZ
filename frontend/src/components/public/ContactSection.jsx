...
function InputField({ name, label, type = "text", required, form, setForm, errors }) {
  ...
  return (
    <div>
      <label
        htmlFor={name}
        className="block text-footnote font-semibold text-[#424245] mb-2 uppercase tracking-wider"
      ...
      />
      {hasError && (
        <p id={`${name}-error`} className="text-caption-1 text-red-500 font-medium mt-1.5" role="alert">
          {errors[name]}
        </p>
      )}
    </div>
...
export default function ContactSection() {
  ...
  return (
    <section id="contact" className="py-20 md:py-28 bg-[#1A1A1A] relative overflow-hidden">
      {/* Subtle orange glow */}
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 w-full relative z-10">
        <div className="grid lg:grid-cols-[1fr_520px] xl:grid-cols-[1fr_560px] gap-12 lg:gap-16">
          ...
          >
            ...
            <h2
              className="font-display font-black text-white mb-6 tracking-[-0.025em] leading-[1.0]"
            ...
            <p
              className="text-[rgba(255,255,255,0.55)] text-callout leading-relaxed mb-10"
            ...
            <div className="space-y-5 mb-10">
              {[
                ...
                { icon: Mail, label: "Email", value: "hello@idaz.vn", href: "mailto:hello@idaz.vn" },
                ...
                >
                  <div
                    className="w-11 h-11 rounded-[13px] flex items-center justify-center flex-shrink-0 transition-colors duration-300 group-hover:bg-[#F5A623] border border-[rgba(255,255,255,0.1)]"
                    style={{ background: "rgba(255,255,255,0.05)" }}
                  >
                    <Icon size={20} className="text-[#F5A623] group-hover:text-[#1D1D1F] transition-colors duration-300" strokeWidth={1.8} />
                  </div>
                  <div>
                    <div className="text-caption-2 text-[rgba(255,255,255,0.35)] font-semibold uppercase tracking-wider mb-0.5" >
                      {label}
                    </div>
                    <div className="text-subheadline font-semibold text-white group-hover:text-[#F5A623] transition-colors duration-200">
                      {value}
                    </div>
                  </div>
                </a>
            ...
            {/* Working hours */}
            <div className="glass-dark rounded-[16px] p-5 border border-[rgba(255,255,255,0.08)]">
              <div className="text-caption-1 font-bold uppercase tracking-wider text-[rgba(255,255,255,0.4)] mb-3" >
                Giờ làm việc
              </div>
              <div className="space-y-2">
                <div className="flex justify-between text-footnote">
                  <span className="text-[rgba(255,255,255,0.6)] font-normal">Thứ 2 – Thứ 6</span>
                  <span className="text-white font-semibold">8:00 – 17:30</span>
                </div>
                <div className="flex justify-between text-footnote">
                  <span className="text-[rgba(255,255,255,0.6)] font-normal">Thứ 7</span>
                  <span className="text-white font-semibold">8:00 – 12:00</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          >
            {submitted ? (
              <div className="bg-white rounded-[24px] p-10 text-center h-full flex flex-col items-center justify-center min-h-[400px]">
                <div
                ...
                <h3 className="font-display font-black text-[#1D1D1F] text-title-2 mb-3 tracking-tight">
                  Gửi thành công! 🎉
                </h3>
                <p className="text-[#6E6E73] leading-relaxed font-normal" >
                  Cảm ơn bạn đã liên hệ. Đội ngũ IDAZ sẽ phản hồi trong vòng 2 giờ làm việc.
                </p>
              </div>
            ) : (
              <form
                onSubmit={handleSubmit}
                className="bg-white rounded-[24px] p-7 md:p-9 flex flex-col gap-5"
                noValidate
              >
                <div>
                  <h3
                    className="font-display font-black text-[#1D1D1F] text-title-3 tracking-tight mb-1"
                  >
                    Điền thông tin tư vấn
                  </h3>
                  <p className="text-footnote text-[#86868B]" >
                    Chúng tôi sẽ liên hệ lại trong 2 giờ làm việc
                  </p>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex items-center justify-center gap-2.5 py-4 rounded-full text-subheadline font-bold transition-all duration-300 hover:shadow-[0_8px_24px_rgba(245,166,35,0.35)] hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                  style={{ background: "var(--color-idaz-orange)", color: "#1D1D1F", }}
                >
                  {loading ? (
                    <>
                      <span className="w-5 h-5 border-2 border-[rgba(29,29,31,0.3)] border-t-[#1D1D1F] rounded-full animate-spin" />
                      Đang gửi...
                    </>
                  ) : (
                    <>
                ...
                <p
                  className="text-caption-1 text-center text-[#C7C7CC] font-normal"
