...
import { Receipt, DollarSign, CreditCard, CheckCircle2, Clock, X, Trash2, Ban, Search, Edit2 } from "lucide-react";
...
export default function BillingDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await userService.getClients('', 100);
      if (res.success) {
        setUsers(res.data);
      }
    } catch (error) {
  ...
  const handleSaveInvoice = async (e) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await invoiceService.updateInvoice(editingId, newInvoice);
      } else {
        res = await invoiceService.createInvoice(newInvoice);
      }
  ...
  const handleExportPDF = (inv) => {
    ...
    setTimeout(async () => {
      let toastId;
      try {
        toastId = toast.loading("Đang tạo PDF...");
